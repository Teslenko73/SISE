...
OUTPUT_DIR  = "./wyniki"
WINDOW_SIZE = 5   # ile ostatnich kroków bierzemy pod uwagę
EPOCHS      = 150
BATCH_SIZE  = 32
...
def detect_columns(df):
    # szukamy kolumn po słowach kluczowych w nazwie
    ...
    if cx is None:  # jeśli nazwy są niestandardowe — bierzemy ostatnie 4
        cx, cy, rx, ry = orig[-4], orig[-3], orig[-2], orig[-1]
    return cx, cy, rx, ry

def get_common_features(df_stat, df_dyn, cx, cy, rx, ry):
    # zostawiamy tylko te cechy UWB, które są w obu zbiorach
...
# 2. FILTR ODRZUCANIA GRUBYCH BŁĘDÓW

def remove_outliers(series, window=5, threshold=500):
    # liczymy medianę kroczącą i zamieniamy punkty mocno od niej odbiegające
...
# 3. OKNO CZASOWE do podania do sieci
...
def create_dataset(df, features, cx_col, cy_col, rx_col, ry_col, window_size):
    ...
    for i in range(len(df) - window_size):
        # cechy UWB + współrzędne z ostatnich window_size kroków
        ...
        # cel: różnica między UWB a referencją w ostatnim punkcie okna
...
def process_sala(sala):
    ...
    # wczytujemy dane statyczne i dynamiczne
    ...
    # uzupełniamy braki w referencji, puste wiersze wyrzucamy
    ...
    # usuwamy grube błędy UWB
    print("-> Aplikuję filtr eliminacji grubych błędów...")
    ...
    # budujemy zbiory z oknem kroczącym
    ...
    model = Sequential([
        ...
        Dense(2, activation='linear')  # przewidujemy poprawkę: delta X, delta Y
    ])
    ...
    # przewidujemy poprawkę i dodajemy do surowych współrzędnych
    ...
    # --- Wykresy i pliki wyjściowe ---
    print("-> Generowanie wykresów i plików do raportu...")

    # A) Dystrybuanta błędu (CDF)
    ...
    # F) Kilka pierwszych wag z każdej warstwy — do wklejenia w raporcie
